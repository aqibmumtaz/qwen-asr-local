...
Default **0.65** (chosen empirically — see Section 6.1 — to flag the
real uncertainty signals on the corpus without false positives on
common words). The comparison is **inclusive** (`<=`), so a word with
`min_conf == 0.65` is flagged. Overridable via env var:
...
elif wc.min_conf >  0.65:  action = "accept_log"
elif wc.min_conf >  0.4:   action = "review"
...
`है।` (0.69) and `आवाज़` (0.73) sit just above the threshold and are not flagged — they're correct, just slightly less confident than mid-sentence variants.
...
Roman Urdu:      aaj ka mausam bahut acha hai. hum ek nai technology ko aazma rahe hain.
...
**Flagged words (Min Conf ≤ 0.65):**

- **`टेक्नोलॉजी (0.61)`** — *the lowest confidence in the sample.* "Technology" phonetically transliterated; 12 BPE tokens (longest word in either sample). Foreign loan + length = many chances for one borderline sub-token. Lexicon corrects: `teknoloji → technology`.

`है।` (0.70) sits just above the 0.65 threshold and is not flagged.
...
At the default `LOW_CONF_THRESHOLD=0.65` with inclusive comparison
(`min_conf <= 0.65 → LOW`), both samples flag exactly the right words —
clean signal-to-noise on this corpus:

- ur1: `अपीब` (0.65), `ज़बान` (0.45), `शनाख्त` (0.56) — 3 flagged
- ur2: `टेक्नोलॉजी` (0.61) — 1 flagged

These are the words a human reviewer should look at: an Arabic proper
name, a near-tied nukta decision, a rare cluster, and a long English
loan word. Sentence-end variants like `है।` (0.69-0.70) sit just above
the threshold and are not flagged — they're correct, just slightly
less confident than mid-sentence variants.

The 0.65 default was chosen empirically; you can tune via env:

```bash
LOW_CONF_THRESHOLD=0.5 python3 ...   # looser — only catches severe cases
LOW_CONF_THRESHOLD=0.8 python3 ...   # stricter — flags moderately uncertain words too
```
...
Capturing logprobs has near-zero overhead at compute time:

1. `output_scores=True` just makes the decoder **retain** the logits
   tensors it already computed during greedy decoding (without this
   flag, the logits get discarded after argmax).
2. Per-token softmax + indexing is a few floating-point ops per token —
   roughly `n_tokens × vocab_size` (152K) memory reads, but no extra
   matmuls.
3. Word grouping + decoding is microseconds (Python list walks).

In short: **confidence extraction is free.** Use it everywhere — there
is no reason to call `hf_asr()` over `hf_asr_with_confidence()` for
performance.
...
code uses, because for words with 12+ tokens (like `टेक्नोलॉजी`) the
direct product can underflow in float32.
...
This is why we use `min_conf`, not `geo_conf`, for the flag — the
average hides the weakest sub-token, which in ASR is often the one
sub-token that determines whether the word is right or wrong.
