...
models=("Qwen3-8B-Q5_K_M" "Qwen3-8B-Q8_0" "Qwen3-8B-Q4_K_M" "Qwen3-4B-Q8_0" "Qwen3-4B-Q4_K_M")
audios=("sample_en1.wav" "sample_en2.wav" "sample_ur1.wav" "sample_ur2.wav")

printf '\xEF\xBB\xBFmodel,audio,asr_seconds,urdu_seconds,inference_seconds,status,transcript,english_translation,urdu_translation\n' > "$CSV"

for m in "${models[@]}"; do
  for a in "${audios[@]}"; do
    tmp=$(mktemp)
    if bash "$ROOT/transcribe_and_translate.sh" --model "$m" "$ROOT/samples/$a" > "$tmp" 2>/dev/null; then
      rc=0
    else
      rc=$?
    fi
    ...
    esc_e=${english//\"/\"\"}
    esc_u=${urdu//\"/\"\"}
    echo "\"$m\",\"$a\",\"$asr_secs\",\"$urdu_secs\",\"$total_secs\",\"$status\",\"$esc_t\",\"$esc_e\",\"$esc_u\"" >> "$CSV"

    cp "$tmp" "$OUT_DIR/run_${m}_${a}.log"
    rm -f "$tmp"

    echo "$m | $a | asr=${asr_secs}s eng=${eng_secs}s urdu=${urdu_secs}s total=${total_secs}s | $status"
  done
done
