...
def prep_16k(src: Path, tag: str) -> Path:
    ...
    if not dst.exists():
        ...
        seg.set_frame_rate(16000).set_channels(1).set_sample_width(2).export(dst, format="wav")
...
def run_asr(wav_path: Path, tag: str) -> dict:
    seg = AudioSegment.from_wav(wav_path).set_frame_rate(16000).set_channels(1).set_sample_width(2)
    ...
    for i in range(n_chunks):
        ...
        print(f"    chunk {i+1:02d}/{n_chunks}  {t_i:5.1f}s  {len(wc_i):3d}w  avg_conf={avg_c:.3f}  →  {text_i[:70]}")
...
def main():
    ...
    for tag, wav_path in wavs.items():
        ...
        r = results[tag]
        print(f"  Done in {elapsed:.1f}s | words={r['n_words']} flagged={r['n_flagged']} "
